...
const screenForNames = (arr) => {
  // Solution code here...
  return arr.filter(Elementent => /^(Mr.|Mrs.|Ms.|Dr.)\s\S/g.test(Elementent));
}
...
const toTitleCase = (arr) => {
  ...
  let newArray = [];
  arr.map(Elementent => {
    newArray.push(Elementent.charAt(0).toUpperCase() + Elementent.slice(1));
  })
  return newArray;
};
...
let biggerThanLuke = (arr) => {
  // Solution code here...
  let Anwser = [];
  let newArray = arr.filter(Element => Number(Element.mass) > Number(arr[0].mass));
  newArray.map(Element => {
    Anwser.push(Element.name);
  });
  return Anwser.join(' - ');
};
